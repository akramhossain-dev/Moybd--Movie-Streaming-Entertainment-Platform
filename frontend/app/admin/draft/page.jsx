...
import { FaFileAlt, FaSearch, FaEdit } from 'react-icons/fa';
    ...
    <AdminLayout>
      <div className="space-y-6">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-black text-foreground flex items-center gap-2">
                <FaFileAlt className="text-warning" /> Draft Movies & Series
              </h1>
              <Badge variant="subtle" size="xs">
                {filteredMovies.length} Drafts
              </Badge>
            </div>
            <p className="text-xs text-foreground-muted">
              Unpublished title entries pending review before release.
            </p>
          </div>
        ...
        {loading ? (
                  ...
                  {filteredMovies.map((m) => (
                    <tr key={m._id} className="hover:bg-surface-elevated/50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-warning/10 text-warning border border-warning/30">
                          Draft
                        </span>
                      </td>
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <DeleteBtn id={m._id} onDeleteSuccess={fetchDrafts} />
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <EmptyState
            title="No draft entries found"
            description="All created movies are currently published."
          />
        )}
      </div>
    </AdminLayout>
